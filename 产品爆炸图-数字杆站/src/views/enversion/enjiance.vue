...
<template>
  ...
  >
    ...
    <div
      ...
      @click="$router.push({ path: '/enjianceone', query: $route.query })"
    ></div>
    <div
      class="jiancetwo_btn cur"
      @click="$router.push({ path: '/enjiancetwo', query: $route.query })"
    ></div>
    <div
      class="jiancethree_btn cur"
      @click="$router.push({ path: '/enjiancethree', query: $route.query })"
    ></div>
    <!-- 返回首页 -->
    <img
...
export default {
  ...
  created() {
    var flat = localStorage.getItem("enjiance");
