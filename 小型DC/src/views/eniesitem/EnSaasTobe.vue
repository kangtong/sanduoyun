<template>
  ...
  >
    ...
    <div class="tobe_btn" @click="$router.push('/ensaasasis')"></div>
...
export default {
  ...
  methods: {
    video1end() {
      ...
      sessionStorage.setItem("ensaastobe", this.video1show);
    },
  },
  created() {
    var flat = localStorage.getItem("ensaastobe");
    console.log(flat);
    flat == "false" ? (flat = false) : (flat = true);
    this.video1show = flat;
    this.video2show = !flat;
    sessionStorage.setItem("ensaastobe", true);
  },
};
</script>
