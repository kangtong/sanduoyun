<template>
  ...
  >
    ...
    <div
      class="tobe_btn"
      @click="$router.push('/enjituanduofenzhizhiasis')"
    ></div>
    <div class="back_btn" @click="$router.push('/enintelligenceies')"></div>
    <div class="home_btn" @click="$router.push('/enhomeview')"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    video1end() {
      ...
      sessionStorage.setItem("enjituanduofenzhi", this.video1show);
    },
  },
  created() {
    var flat = localStorage.getItem("enjituanduofenzhi");
    console.log(flat);
    flat == "false" ? (flat = false) : (flat = true);
    this.video1show = flat;
    this.video2show = !flat;
    sessionStorage.setItem("enjituanduofenzhi", true);
  },
};
</script>
